...
| 文件 | 控制内容 | 通常刷写 |
| :--- | :--- | :--- |
| `config/sweep.keymap` | 层、按键绑定、旋钮绑定、鼠标层绑定、指针处理器、`Z` 按住滚动、`X` 按住左键、`Mode` 模式切换。 | 通常刷左手主控；如果改到右手分体行为，也构建右手。 |
...
| `config/sweep_left_display_hw.conf` | 显示屏、LVGL、SSD16XX、e-ink 渲染相关配置。 | 左手带屏幕固件。 |
...
| 修改显示屏固件能力 | `config/sweep_left_display_hw.conf` | 是否启用显示、LVGL、显示线程、字体和内存池等编译配置在这里。 |
