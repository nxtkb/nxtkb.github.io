...
| 触控板行 | 触控板状态 | 运行时 Cirque 模式和速度档位，例如 `Trackpad: A P2 S1`。`A` 表示绝对模式，`R` 表示相对模式，`P` 是指针速度档位，`S` 是滚动速度档位。 |
...
| 触控板 | `zmk_trackpad_status_changed`，以及来自触控板/pointing 模块的 Cirque 模式、指针速度档位和滚动速度档位。 |
